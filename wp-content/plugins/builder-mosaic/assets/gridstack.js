...
(function($, scope) {
  const Utils = {
    ...
    updateScrollPosition: function(el, ui, distance) {
      ...
      var rect = el.getBoundingClientRect(),
        innerHeightOrClientHeight = (window.innerHeight || document.documentElement.clientHeight);
      if (rect.top < 0 ||
        ...
        offsetDiffUp = rect.top,
        scrollEl = Utils.getScrollParent(el);
        if (scrollEl !== null) {
  ...
  */
  ...
  const GridStackEngine = function(column, onchange, float, maxRow, items) {
  ...
  GridStackEngine.prototype._fixCollisions = function(node) {
    this._sortNodes(-1);
    var nn = node,
     hasLocked = Boolean(this.nodes.find(function(n) { return n.locked; }));
  ...
  GridStackEngine.prototype.isAreaEmpty = function(x, y, width, height) {
    ...
    collisionNode = this.nodes.find(function(n) {
  ...
  GridStackEngine.prototype.moveNode = function(node, x, y, width, height, noPack) {
    ...
    nn = { x: x, y: y, width: width, height: height,
  ...
  var GridStack = function(el, opts) {
    var self = this,
    oneColumnMode, _prevColumn, isAutoCellHeight;
    ...
    this.engine = new GridStackEngine(this.opts.column, function(nodes, detachNode) {
      ...
      nodes.forEach(function(n) {
        if (detachNode && n._id === null) {
          ...
          $(n.el).attr({'data-gs-x': n.x,'data-gs-y': n.y,'data-gs-width': n.width,'data-gs-height': n.height});
        }
      });
    ...
    this.onResizeHandler = function() {
        ...
        if (!self.opts.disableOneColumnMode && window.innerWidth <= self.opts.minWidth) {
	...
	if (!self.opts.disableWindowResize) {
    ...
    if (!self.opts.staticGrid && self.opts.acceptWidgets) {
      var draggingElement = null,
       ...
       onDrag = function(event, ui) {
        ...
        node = el.data('_gridstack_node'),
        pos = self.getCellFromPixel({left: event.pageX, top: event.pageY}, true),
        x = Math.max(0, pos.x),
        y = Math.max(0, pos.y);
        if (!node._added) {
          node._added = true;
          ...
          self.placeholder
            .attr({'data-gs-x': node.x,'data-gs-y': node.y,'data-gs-width':node.width,'data-gs-height': node.height})
            .show();
      ...
      this.dd
        ...
        .on(self.$el, 'dropover', function(event, ui) {
          ...
          el.data({'_gridstack_node': node,'_gridstack_node_orig': origNode}).on('drag', onDrag);
        ...
        .on(self.$el, 'drop', function(event, ui) {
          ...
          $(ui.draggable).unbind('drag', onDrag).removeData('_gridstack_node').removeData('_gridstack_node_orig');
  ...
  GridStack.prototype._initStyles = function() {
    ...
    const head=this.opts.inHead?document.getElementsByTagName('head')[0]:this.el.parentNode;
    this._styles = Utils.createStylesheet(this._stylesId, head);
  ...
  GridStack.prototype._updateContainerHeight = function() {
    ...
    if (cssMinHeight > 0) {
      var verticalMargin = this.opts.verticalMargin,
        minRow =  Math.round((cssMinHeight + verticalMargin) / (this.cellHeight() + verticalMargin));
      if (row < minRow) {
        row = minRow;
      }
    }
  ...
  GridStack.prototype._prepareElementsByNode = function(el, node) {
    var self = this,
     ...
     cellWidth,
     cellFullHeight; // internal cellHeight + v-margin

    var dragOrResize = function(event, ui) {
       var x = Math.round(ui.position.left / cellWidth),
       y = Math.floor((ui.position.top + cellFullHeight / 2) / cellFullHeight),
       width,
       height;
      ...
      if (event.type === 'drag') {
        ...
        if (el.data('inTrashZone') || x < 0 || x >= self.engine.column || y < 0 ||
          ...
          if (node._temporaryRemoved) {
            self.engine.addNode(node);
            self.placeholder
              .attr({'data-gs-x': x,'data-gs-y': y,'data-gs-width':width,'data-gs-height': height})
              .show();
            self.$el.append(self.placeholder);
            node.el = self.placeholder.get(0);
      ...
      var lastTriedWidth = width !== undefined ? width : node.lastTriedWidth,
       lastTriedHeight = height !== undefined ? height : node.lastTriedHeight;
    ...
    onStartMoving = function(event, ui) {
      ...
      self.placeholder
        .attr({'data-gs-x': o.attr('data-gs-x'),'data-gs-y': o.attr('data-gs-y'),'data-gs-width': o.attr('data-gs-width'),'data-gs-height':o.attr('data-gs-height')})
        .show();
      node.el = self.placeholder.get(0);
      node._beforeDragX = node.x;
      node._beforeDragY = node.y;
    ...
    onEndMoving = function(event, ui) {
      ...
      if (node._isAboutToRemove) {
        ...
        el.removeData('_gridstack_node').remove();
      } else {
        self._clearRemovingTimeout(el);
        if (!node._temporaryRemoved) {
          ...
          Utils.removePositioningStyles(o);
          o
            .attr({'data-gs-x': node._beforeDragX,'data-gs-y': node._beforeDragY,'data-gs-width': node.width,'data-gs-height':node.height});
      ...
      if (nestedGrids.length && event.type === 'resizestop') {
        ...
        o.find('.grid-stack-item').trigger('resizestop').trigger('gsresizestop');
      }
  ...
  GridStack.prototype.addWidget = function(el, opt, y, width, height, autoPosition, minWidth, maxWidth, minHeight, maxHeight, id) {
    ...
    if (opt) { // see knockout above
      ...
      Utils.defaults(opt, this._readAttr(el));
  ...
  GridStack.prototype.cellHeight = function(val, noUpdate) {
    ...
    if (val === undefined) {
      ...
      var o = this.$el.children('.' + this.opts.itemClass).first(),
       ...
       verticalMargin = this.opts.verticalMargin;
  ...
  GridStack.prototype.getCellFromPixel = function(position, useOffset) {
    ...
    relativeLeft = position.left - containerPos.left,
    relativeTop = position.top - containerPos.top,

    columnWidth = Math.floor(this.$el.width() / this.opts.column),
    rowHeight = Math.floor(this.$el.height() / parseInt(this.$el.attr('data-gs-current-row')));
  ...
  GridStackEngine.prototype._updateNodeWidths = function(oldColumn, column, nodes) {
    ...
    lastIndex = this._layouts.length - 1;
  ...
  GridStack.prototype.column = function(column, doNotPropagate) {
    ...
    this.$el.removeClass('grid-stack-' + oldColumn).addClass('grid-stack-' + column);
  ...
  GridStack.prototype.off = function(eventName,callback) {
    // check for array of names being passed instead
    if (eventName.indexOf(' ') !== -1) {
      var names = eventName.split(' ');
