...
class PTB_Custom_Post_Type {
    ...
    const AD_SHOW_IN_REST = 'show_in_rest'; // whether REST API should be enabled for this post type
    ...
    public function get_args() {
        ...
        $rewrite_slug = PTB_Utils::get_label($this->ad_rewrite_slug);

        $labels = array(
            'name' => $plural_label,
            'singular_name' => $singular_label,
            'menu_name' => PTB_Utils::safe_sprintf($menu_name, $plural_label),
            'name_admin_bar' => $singular_label,
            'add_new' => $add_new,
            'add_new_item' => PTB_Utils::safe_sprintf($add_new_item, $singular_label),
            'new_item' => PTB_Utils::safe_sprintf($new_item, $singular_label),
            'edit_item' => PTB_Utils::safe_sprintf($edit_item, $singular_label),
            'view_item' => PTB_Utils::safe_sprintf($view_item, $singular_label),
            'all_items' => PTB_Utils::safe_sprintf($all_items, $plural_label),
            'search_items' => PTB_Utils::safe_sprintf($search_items, $plural_label),
            'parent_item_colon' => PTB_Utils::safe_sprintf($parent_item_colon, $plural_label),
            'not_found' => $not_found,
	...
	/**
	 * Get admin URL to post type's edit screen
	 *
	 * @return string
	 */
	public function get_edit_url() {
		return admin_url( 'admin.php?page=ptb-cpt&action=edit&post_type=' . $this->slug );
	}
}
