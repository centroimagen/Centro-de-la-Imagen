 ...
 */
class PTB_CMB_Audio extends PTB_CMB_Base {
    ...
    public function __construct($type, $plugin_name, $version) {
	    ...
	    if(!is_admin() || (defined('DOING_AJAX') &&  DOING_AJAX)){
		    add_filter('ptb_meta_audio_exist', array($this,'audio_exist'),10,3);
	    }
    }
	...
	public function get_assets() {
		return [
			'css' => [
				self::$plugin_name . '-' . $this->type => PTB::$uri . 'public/css/modules/audio.css',
			],
		];
	}
            ...
            <ul id="<?php echo $meta_key; ?>_options_wrapper" class="ptb_cmb_options_wrapper">
                ...
                <?php foreach ($values as $index => $v): ?>
                    ...
                    <li class="<?php echo $meta_key; ?>_option_wrapper ptb_cmb_option">
                        ...
                        <input type="text" placeholder="Audio file URL (mp3)" name="<?php echo $url_name; ?>" value="<?php echo esc_url_raw($v); ?>"/>
    ...
    public function ptb_submission_form($post_type, array $args, array $module, $post, $lang, $languages) {
        PTB_Utils::enqueue_script( self::$plugin_name . '-submission-audio', PTB_Submission::$url . 'public/js/field-audio.js', array( 'ptb-submission' ), $this->get_plugin_version(), true );
            ...
            <ul>
                <?php foreach ($data['url'] as $k => $v): ?>
                    ...
                    <li class="ptb-submission-text-option">
                        ...
                        <div class="ptb_back_active_module_input">
                            <div class="ptb_extra_image_title"><label data-label="<?php _e('Upload Audio', 'ptb') ?>" class="ptb-submission-upload-btn" for="ptb_submission_<?php echo $args['key'] ?>"><span class="ptb-submission-upload-btn-iconup"><?php echo PTB_Utils::get_icon( 'fa-upload' ); ?></span><?php echo $v?pathinfo($v,PATHINFO_FILENAME):__('Upload Audio', 'ptb') ?></label></div>
                            <?php PTB_CMB_Base::module_language_tabs('submission', isset($title[$k]) ? $title[$k] : array(), $languages, $args['key'] . '_title', 'text', __('Audio Title', 'ptb'), true); ?>
                        ...
                        <?php PTB_CMB_Base::module_language_tabs('submission', isset($desc[$k]) ? $desc[$k] : array(), $languages, $args['key'] . '_description', 'textarea', __('Audio Description', 'ptb'), true); ?>
	 ...
	 * @return array
	 */
	public function audio_exist ( array $meta_query, $origk, array $cmb_option) {
