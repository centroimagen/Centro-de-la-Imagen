...
class PTB_List_Table_PTT extends WP_List_Table {
    ...
    function column_name($item) {
        //Build row actions
        $actions = [];
		if ( post_type_exists( $item['post_type'] ) ) {
			$actions['edit'] = sprintf('<a href="?page=%s&action=%s&ptb-ptt=%s">%s</a>', $_REQUEST['page'], 'edit', $item['ID'], __( 'Edit', 'ptb' ) );
		} else {
			$actions['null'] = __( 'Missing Post Type', 'ptb' );
		}
		$actions['delete'] = sprintf('<a href="?page=%s&action=%s&ptb-ptt=%s&nonce=%s">Delete</a>', $_REQUEST['page'], 'delete', $item['ID'], self::$nonce);
