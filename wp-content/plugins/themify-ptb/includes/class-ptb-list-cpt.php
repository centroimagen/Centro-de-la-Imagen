...
class PTB_List_Table_CPT extends WP_List_Table {
    ...
    private function prepare_data() {
		...
		foreach ( PTB_Utils::get_public_post_types() as $type ) {
			$cpt_data = array();
			$type = get_post_type_object( $type );
			$cpt_data['ID'] = $type->name;
            $cpt_data['singular_label'] = $type->labels->singular_name;
            $cpt_data['description'] = $type->description;
            $cpt_data['slug'] = $type->name;
            $cpt_data['supports'] = '';
            $cpt_data['taxonomies'] = '';
			$this->data[] = $cpt_data;
		}
    ...
    function column_singular_label($item) {
		...
		/* these actions are only available for post types regstered by PTB */
		if ( $this->options->has_custom_post_type( $item['ID'] ) ) {
			...
			$template = $this->options->get_post_type_template_by_type( $item['ID'] );
			if ( $template ) {
				$actions = array_merge( $actions, [
					'edit_template' => sprintf(
						'<a href="%1$s" title="%2$s">%2$s</a>', admin_url( 'admin.php?page=ptb-ptt&action=edit&ptb-ptt=' . $template->get_id() ), __( 'Edit Template', 'ptb' )
					),
				] );
			}
		}
