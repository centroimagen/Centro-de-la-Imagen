...
class PTB_Form_CTX {
    ...
    public function add_settings_fields($slug_admin_ctx) {
        ...
        add_settings_section(
                $this->settings_section_cl, $icon . __('Custom Labels', 'ptb'), array($this, 'cl_section_cb'), $this->slug_admin_ctx
        );
        ...
        add_settings_section(
                $this->settings_section_ad, $icon . __('Advanced Options', 'ptb'), array($this, 'ad_section_cb'), $this->slug_admin_ctx
        );
    ...
    public function ctx_taxonomy_cb() {
        ...
        echo $this->generate_input_text(
                        PTB_Custom_Taxonomy::SLUG, $this->ctx->slug, false, __('(eg. "producers". Should only contain lowercase english characters and underscore - no space or special characters)', 'ptb')
        );
    }
    ...
    public function ctx_attach_to_cb() {
        ...
        printf(
                '<fieldset class="tf_clearfix"><div class="ptb-pull-left">%1$s</div><div class="ptb-pull-left">%2$s</div></fieldset>', implode('', $arrays[0]), implode('', $arrays[1])
        );
        printf(__('Assign this taxonomy to above post type(s) %s.', 'ptb'), '<a href="//codex.wordpress.org/Function_Reference/register_taxonomy" target="_blank">' . __('(learn more)', 'ptb') . '</a>');
    }
    ...
    private function add_fields_advanced_options() {
        ...
        $fields = array(
            ...
            PTB_Custom_Taxonomy::AD_SHOW_IN_REST => __('Show in REST API', 'ptb'),
            PTB_Custom_Taxonomy::AD_REWRITE_SLUG => __('Rewrite Slug', 'ptb'),
            PTB_Custom_Taxonomy::AD_REWRITE_FRONT => __('Rewrite With Front', 'ptb'),
        );
    ...
    public function add_fields_advanced_options_cb($args) {
        ...
        switch ($referrer) {
			...
			case PTB_Custom_Taxonomy::AD_SHOW_IN_REST :

                echo $this->generate_input_radio_yes_no(
                                PTB_Custom_Taxonomy::AD_SHOW_IN_REST, $this->ctx->ad_show_in_rest, __('Whether to include the taxonomy in the REST API. (Require for display in Gutenberg editor)', 'ptb')
                );
                break;

			case PTB_Custom_Taxonomy::AD_REWRITE_SLUG :

				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::AD_REWRITE_SLUG,
					$this->ctx->rewrite_slug,
					false,
					__( 'Default will use taxonomy\'s name.', 'ptb' )
				) );
                break;

			case PTB_Custom_Taxonomy::AD_REWRITE_FRONT :

                echo $this->generate_input_radio_yes_no(
					PTB_Custom_Taxonomy::AD_REWRITE_FRONT, $this->ctx->with_front, __('Whether the permalink structure be prepended with the front base.', 'ptb')
                );
                break;
        }
    }
    ...
    private function extract_data($input) {
        ...
        $this->ctx->ad_show_in_rest = ( 'Yes' === sanitize_text_field($input[$this->get_field_id(PTB_Custom_Taxonomy::AD_SHOW_IN_REST)]) );
        $this->ctx->rewrite_slug = sanitize_text_field( $input[$this->get_field_id(PTB_Custom_Taxonomy::AD_REWRITE_SLUG)] );
        $this->ctx->with_front = ( 'Yes' === sanitize_text_field($input[$this->get_field_id(PTB_Custom_Taxonomy::AD_REWRITE_FRONT)]) );
    }
    ...
    private function generate_input_text($id, $value, $hidden = false, $description = '') {
        ...
        $input = '';
        if ( $hidden || in_array( $id, [PTB_Custom_Taxonomy::SLUG, PTB_Custom_Taxonomy::AD_REWRITE_SLUG] ) ) {
