(($,doc, win)=>{
	...
	const callback = ()=>{
		...
		/* compatibility for Themify Builder frontend editor */
		if ( typeof ptbWidget !== 'undefined' ) {
			win.top.Themify.loadCss( ptbWidget.css );
			body = win.top.document.body;
			ajaxurl = themify_vars.ajax_url;
		}
        ...
        $( body ).on('change', '.ptb_widget_resent_posts', function () {
            var self = $(this),
                ...
                $name = self.data('name'),
                $id = self.data('id'),
				in_builder = self.closest( '.tb_field.instance_widget' ).length;
                        ...
                        if (resp.tax && resp.tax.data) {
                            ...
                            for (var i in tax_data) {
                                if(tax_data[i].type==='multiselect'){
									...
									/* required for saving in Themify Builder */
									if ( ! in_builder ) {
										name += '[]';
									}
	...
	if ( doc.readyState === 'complete' ) {
		...
		win.addEventListener( 'pageswhow', callback,{passive:true,once:true} );
	}
