(function ($) {
    ...
    $(document).on('ptb_metabox_create_progress_bar', function (e) {
        ...
        $.each(e.options.options, function (index, option) {
            ...
            $newOption.find('.' + e.id + '_remove').on( 'click', {option: $newOption}, removeOption);
        ...
        e.container.find('#' + e.id + '_add_new')
			...
			function (e) {
				...
				$newOption.find('input[name^="' + e.data.id + '_options_"]')
						...
						.data('id', nextId);
				$newOption.find('.' + e.data.id + '_remove').on( 'click', {option: $newOption}, removeOption);
        ...
        function getNextId(e) {
            var set = $( e.target ).closest( '.ptb_cmb_input_row' ).find('input[name^="' + e.data.id + '_options_"]').map(function () {
                return $(this).data('id');
            });
    ...
    }).on('ptb_add_metabox_progress_bar', function (e) {
        return {
    ...
    }).on('ptb_metabox_save_progress_bar', function (e) {
	...
	window.addEventListener('DOMContentLoaded', function(){
        $('#ptb_lightbox_container').on('click', 'div[data-type="progress_bar"] fieldset legend', function (e) {
            e.preventDefault();
