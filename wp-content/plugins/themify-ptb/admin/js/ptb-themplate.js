    ...
    PTB = {
        ...
        Open: function () {
            $(document).on('click', '.' + PTB.prefix + 'toggle_module,.' + PTB.prefix + 'toggle_row', function (e) {
                var $container = $(this).closest('.' + PTB.prefix + 'active_module').length === 0 ?
                    ...
                    $toggle = $container.closest('.'+PTB.prefix+'back_module.'+PTB.prefix+'dragged,.'+PTB.prefix+'back_row');
                ...
                if ($(this).hasClass(PTB.prefix + 'opened') || $container.is(':visible')) {
                    ...
                    $toggle.removeClass(PTB.prefix + 'expanded');

                    if ($col.length > 0 && $col.find('.' + PTB.prefix + 'opened').length == 0) {
                        $col.removeClass('shadow', 200, function () {
                            $container.slideUp(200, function () {
                                $col.removeClass('fill', 200, 'swing', function () {
                                    $col.removeClass('fill');
                                    $col.siblings(activeColSelector).show(200);
                    ...
                    $toggle.addClass(PTB.prefix + 'expanded');
                    if ($col.length > 0 && $col.siblings(activeColSelector).length !== 0) {
                        $col.siblings(activeColSelector).hide(200, function () {
                            $col.addClass('fill');
        ...
        Delete: function () {
            $(document).on('click', '.' + PTB.prefix + 'delete_module', function (e) {
                if (confirm(ptb_js_admin.module_delete)) {
                    var $container = $(this).closest('.' + PTB.prefix + 'active_module').length === 0 ?
        ...
        AddRow: function () {
            $('.' + PTB.prefix + 'add_row').click(function (e) {
				...
				$row.addClass( 'ptb_expanded' )
					.find('.ptb_back_row_content').show()
					.prev().find( '.ptb_toggle_module' ).addClass( 'ptb_opened' );
                ...
                setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res && res.status == '1') {
                                $('#' + PTB.prefix + 'success_text').addClass('ptb_success');
                                setTimeout(function () {
                                    //  $('.' + PTB.prefix + 'close_lightbox').trigger('click');
                                    $('#' + PTB.prefix + 'success_text').removeClass('ptb_success');
                                }, 2000);
        ...
        ParseData: function () {
            ...
            $rows.each(function (i) {//each rows
                ...
                $col.each(function (k) {//each column in row
                    ...
                    $data[i][$key] = {};
                    $data[i][$key]['col_classes'] = $.trim($(this).find('.'+ PTB.prefix+'col_custom_css').val());
                    var $modules = $(this).find('.' + PTB.prefix + 'back_active_module_content');
                    $modules.each(function (j) {//each module in column
						...
						/* for custom template items that don't have any options */
						if ( ! $inputs.length ) {
							$data[i][$key][j]['key'] = $type;
						}
		...
		showHideOptions : function() {
			$( document ).on( 'change', '.ptb_thousand_separator input[type="text"]', function() {
				$( this ).closest( '.ptb_back_active_module_row' ).nextAll( ':lt(2)' ).toggle( this.value !== '' );
			} );
			$( '.ptb_thousand_separator input[type="text"]' ).trigger( 'change' );
		},
        ...
        AddIcon: function () {
            $(document).on('click', '.ptb-icons-list a', function (e) {
                e.preventDefault();
                var $current = $('#ptb_row_wrapper .ptb_current_ajax'),
                    $class = $(this).data('icon');
                $current.prev('input').val( $class );
