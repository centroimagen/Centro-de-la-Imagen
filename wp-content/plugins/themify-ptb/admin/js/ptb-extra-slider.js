(function ($) {
    ...
    function selectImage(attachment, $id, $current) {
        ...
        var $new = $('#auto_draft').length > 0,
                $parent = $current ? $current : $('#' + $id + '_options_wrapper li.ptb_cmb_option').last(),
    ...
    $(document).on('ptb_add_metabox_slider', function (e) {
        return {};
    }).on('ptb_post_cmb_slider_body_handle', function (e) {
        ...
        $('.ptb_post_cmb_item_slider').on( 'click','.' + e.id + '_option_wrapper .ptb_post_cmb_image', function (event) {
