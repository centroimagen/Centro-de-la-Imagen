...
class DMS
{
    ...
    public static function run()
    {
        ...
        if ( !is_admin() ) {
            ...
            if ( strpos( $DMS->domain, $DMS->base_host ) === 0 ) {
                return;
            }
            ...
            if ( $DMS->domain_path_match ) {
                ...
                if ( $global_mapping_on && $DMS->dms_fs->can_use_premium_code__premium_only() ) {
                    $main_mapping = $DMS->getMainMappingDomain();
                    if ( !empty($main_mapping->host) && ($DMS->domain != $main_mapping->host || !empty($DMS->path) && !empty($main_mapping->path) && strpos( $DMS->path, $main_mapping->path ) !== 0) ) {
                    ...
                    if ( !empty($DMS->path) && !empty($main_mapping->path) ) {
                        $real_path = ( strpos( $DMS->path, $main_mapping->path ) === 0 ? trim( $DMS->strReplaceOnce( $main_mapping->path, '', $DMS->path ), '/' ) : null );
    ...
    public function addActions()
    {
        ...
        add_action( 'admin_post_save_dms_config', array( $this, 'saveConfig' ) );
        add_action( 'wp_ajax_dms_hide_sunrise_note', array( $this, 'hideSunriseNotice' ) );
    }
     ...
     * @return void
     */
    public static function hideSunriseNotice()
    {
        DMS_Helper::hideSunriseNotice();
    }
    ...
    {
        // Check nonce
        check_admin_referer( 'save_dms_config_action', 'save_dms_config_nonce' );
    ...
    public function saveMapping()
    {
        ...
        if ( empty($this->platform) || $this->platform->allowMappingSave() ) {
            ...
            $dms_main_domain_post_data = ( isset( $_POST['dms_main_domain'] ) ? $_POST['dms_main_domain'] : null );
            ...
            if ( $config_exists ) {
                ...
                foreach ( $domains as $key => $item ) {
                    ...
                    if ( !empty($item['custom_html']) && $this->dms_fs->can_use_premium_code__premium_only() ) {
    ...
    public static function forceRedirect()
    {
        ...
        if ( $DMS->dms_fs->can_use_premium_code__premium_only() && !isset( $_GET['elementor-preview'] ) ) {
            ...
            if ( DMS_Helper::isSubDirectoryInstall() && strpos( $DMS->path, $path ) === false ) {
             ...
             */
            if ( $match && (!empty($path) && strpos( $DMS->path, $path ) === false || empty($path)) ) {
                self::redirectTo(
                    $host,
                    $path,
                    $DMS->path,
     ...
     * @since 1.6
     */
    public function getMainMappingDomain()
    {
        $wpdb = $this->wpdb;
        return $wpdb->get_row( "SELECT `host`, `path`, `attachment_id` FROM `" . $wpdb->prefix . "dms_mappings` AS m ORDER BY m.main DESC, m.id ASC LIMIT 1" );
    }
            ...
            if ( !empty($DMS->path) && $DMS->domain_path_match ) {
                ...
                if ( empty($key) ) {
                    ...
                    if ( $DMS->dms_fs->can_use_premium_code__premium_only() && $global_mapping_on ) {
                        $main_mapping = $DMS->getMainMappingDomain();
                        if ( !empty($main_mapping->host) && $DMS->domain != $main_mapping->host && strpos( $DMS->path, $main_mapping->path ) !== 0 ) {
                            self::redirectTo(
                                $main_mapping->host,
                                $main_mapping->path,
                                null,
                    ...
                    $wp_query->set_404();
                    status_header( 404 );
                } elseif ( $DMS->dms_fs->can_use_premium_code__premium_only() && $key == $DMS->map[0]['primary'] && !is_null( $DMS->map[0]['path'] ) && ($DMS->map[0]['path'] === '' || strpos( $DMS->path, $DMS->map[0]['path'] ) === 0) && $DMS->path != $DMS->map[0]['path'] ) {
                    /**
    ...
    public static function prepareUriFilters()
    {
        ...
        if ( !is_admin() ) {
            ...
            // When front page is active, then home_url filter is enough
            ...
            if ( !empty(get_option( 'page_on_front' )) ) {
                ...
                // Otherwise list all possible filters
    ...
    {
        //TODO store this (get_option( 'dms_rewrite_urls_on_mapped_page' )) value at the beginning in some property. Or refactor this some intuitively
        ...
        if ( ($this->domain_path_match || $this->is_main_mapping_with_path) && $this->dms_fs->can_use_premium_code__premium_only() && !empty($this->url_rewrite) ) {
             ...
             */
            //TODO maybe we need to get real front page path
    ...
    public function getRewrittenUrl( $key, $link )
    {
        ...
        if ( !empty($this->selective_url_rewrite) ) {
            ...
            //TODO check weather we could reduce get_option call to one ( but I think it is being cached somehow )
            if ( empty($mapping) && !empty(get_option( 'dms_global_mapping' )) ) {
                // Proceed with global mapping
                $mapping = $this->getMainMappingDomain();
            }
            if ( !empty($mapping->host) ) {
                $mapped_link = str_ireplace( $host, $mapping->host . (( !empty($mapping->path) ? '/' . $mapping->path : '' )), $link );
            }
    ...
    public static function showCustomHtml()
    {
        ...
        if ( !empty($DMS->map_custom_html) ) {
            echo  stripslashes( $DMS->map_custom_html ) ;
        }
    }
