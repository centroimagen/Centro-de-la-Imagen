...
class PTB_Search_Admin {
    ...
    public function __construct($plugin_name, $version, $options) {
		...
		add_action( 'ptb_troubleshoot', array( $this, 'ptb_troubleshoot' ) );
		add_action( 'load-post-type-builder_page_ptb-troubleshoot', array( $this, 'flush_cache' ) );
    }
    ...
    public function enqueue_scripts() {
        ...
        if (in_array($screen->id, $screens)) {
            ...
            wp_enqueue_script($this->plugin_name,PTB_Utils::enque_min(  $pluginurl . 'admin/js/ptb-search.js'), array( 'jquery', 'ptb-admin' ), $this->version, false);
    ...
    public function add_plugin_admin_menu($menu) {
        $menu[$this->plugin_name] = array(__('Search Forms', 'ptb-search'), __('Search Forms', 'ptb-search'), 'manage_options', array($this, 'display_list'));
    ...
    public function save_temlate() {
        ...
        if (isset($_POST['post_type']) && isset($_POST['title']) && current_user_can('manage_options')) {
            ...
            $cpt_id = str_replace(' ', '_', strtolower($title));
            $ptb_options = PTB::get_option();
    ...
    public function ptb_search_modules(array $cmb_options, $type, $post_type) {
        if ($type === 'search') {
            unset($cmb_options['permalink'],$cmb_options['editor'],$cmb_options['excerpt'],$cmb_options['thumbnail'],$cmb_options['comments'],$cmb_options['comment_count']);
            $cmb_options['date']['name'] =PTB_Search_Options::get_name('date');
            foreach ($cmb_options as $id => $v) {
                if (in_array($v['type'], array('gallery', 'slider', 'file', 'icon', 'audio', 'video', 'image', 'relationship'))) {
                    unset($cmb_options[$id]);
                }
            }
            $cmb_options['button'] = array('type' => 'button', 'name' => PTB_Search_Options::get_name('button'));
            $cmb_options['has'] = array('type' => 'has', 'name' => PTB_Search_Options::get_name('has'));
            $cmb_options['content'] = array('type' => 'content', 'name' => PTB_Search_Options::get_name('content'));
        }
        return $cmb_options;
    }
    ...
    public function ptb_template_save(array $post_data, array $data) {
        if (isset($data['ptb_type']) && $data['ptb_type'] === 'search') {
            $_keys = array( 'result_type', 'page','no_result', 'orderby', 'order', 'scrollto', 'disable_ajax' ) ;
	...
	public function ptb_troubleshoot() {
		?>
		<div class="card">
			<h3 class="title"><?php _e( 'PTB Search', 'ptb-search' ); ?></h3>
			<p><?php _e( 'Clear the PTB Search Cache', 'ptb-search' ); ?></p>
			<p><a href="<?php echo add_query_arg( [ 'action' => 'ptb-search-flush', 'nonce' => wp_create_nonce( 'ptb_search_nonce' ) ] ); ?>" class="button button-primary"><?php _e( 'Flush Cache', 'ptb' ); ?></a></p>
		</div>
		<?php
	}

	public function flush_cache() {
		if ( isset( $_GET['action'] ) && $_GET['action'] === 'ptb-search-flush' && wp_verify_nonce( $_GET['nonce'], 'ptb_search_nonce' ) ) {
			add_settings_error( 'ptb-troubleshoot', '', __( 'PTB Search cache has been cleared.', 'ptb-search' ), 'updated' );
			PTB_Search_Options::remove_cache();
		}
	}
}