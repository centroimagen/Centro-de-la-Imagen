    ...
    const getPos=el=>{
                            ...
                            if (type) {
                                ...
                                if (type === 'lightbox') {
                                    const config = link.dataset.zoomConfig.split('|'),
                                            currentUW = config[0].includes('%') ? '%' : 'px',
                                            currentUH = (config[1] && config[1].includes('%') ) ? '%' : 'px';
                    ...
                    result(el) {
                        ...
                        if (value) {
                            ...
                            if (value.includes('rgb')) {
                                opacity = value.includes('rgba') ? parseFloat(value.split(',').slice(-1).pop()) : 1;
            ...
            imageEditing = item=> {
                ...
                if(imageToolbar===null){
                    ...
                    const rect = getPos(item),
                        ...
                        help = !src.includes(Themify.upload_url)  && !src.includes('blob:')? themifyBuilder.i18n.img_help : (activeEl.closest('[data-hasEditor]') !== null ? themifyBuilder.i18n.img_help2 : false),
                        handlers = ['w', 's', 'e', 'n'],
            ...
            imageResizing = async function (e) {
                ...
                if (activeEl && (activeEl.hasAttribute('data-w') || activeEl.hasAttribute('data-h') || activeEl.closest('[data-hasEditor]')) && (target.classList.contains('image_handler') || target.classList.contains('image_border'))) {
                    ...
                    const axis = target.dataset.axis,
                            ...
                            mouseup = async function (e) {
                                ...
                                this.tfOff('pointermove', move, {passive: true})
                                    .tfOff('lostpointercapture pointerup', mouseup, {passive: true, once: true});
                    ...
                    target.tfOn('lostpointercapture pointerup', mouseup, {passive: true, once: true});
                }
            },
        /*
