((Themify, win, topWindow, doc, api,und) => {
    ...
    ThemifyLiveStyling.prototype.transform = function (el, id) {
        ...
        for (let options = ['scale', 'translate', 'rotate', 'skew'],i = 0; i < options.length; ++i) {
            let type = options[i];
            switch (type) {
                case 'scale':
                case 'translate':
                case 'skew':
                    ...
                    break;
                case 'rotate':
                    for (let inputs = ['z', 'y', 'x'],j = inputs.length - 1; j > -1; --j) {
        ...
        el.tfOn(event, function (e) {
            ...
            if (is_select === true) {
                ...
                if (this.closest('#background_video_options') !== null) {
                    ...
                    return;
                } else if (('height' === prop || 'width' === prop) && id.includes('_auto_'+prop)) {
                    const mainID =data.origId;
                    if (this.checked) {
                    ...
                    const selector = '#' + data.posId,
                            wrap = this.closest('.tb_multi_fields');
                    if (this.checked) {
                        val = 'auto';
                    } else {
