...
class TB_Fancy_Heading_Module extends Themify_Builder_Component_Module {
		...
		return array(
			...
			array(
				...
				'options' => array(
					...
					array('value' => 'image_icon', 'name' => 'image'),
	...
	protected function _visual_template() {
		...
		type = data.icon_type || 'icon',
		icon=type!='image'?(type!='l'?(data.icon ? api.Helper.getIcon(data.icon).outerHTML:'') : (api.Helper.getLottie(data,'parent').outerHTML || '')):data.image;
		icon_style = data.icon_c && type=='icon'? 'style="color: ' + api.Helper.toRGBA( data.icon_c ) + '"' : '',
		heading_tag = data.heading_tag || 'h1',
