...
((doc, Themify, und) => {
    ...
    const cacheLoaded = new Map,
            ...
            initCustomElement = el => {
                if (!el.shadowRoot && !done.has(el)) {
                    const p = el.tfTag('template')[0] || el.tfTag('script')[0],
                            args = p?.content.textContent || el.dataset.args;
                    if (args) {
						done.add(el);
    ...
    class LottieElement extends HTMLElement {
        ...
        attributeChangedCallback(attribute, previousValue, currentValue) {
            if ((!currentValue && attribute === 'data-lazy') || Themify.is_builder_active) {
                initCustomElement(this);
            }
        }
    ...
    TF_Lottie = class {
        ...
        isLive() {
            if (!this.el?.isConnected || !this.player?._cbs) {
                this.destroy();
                return false;
            }
            return true;
        ...
        run() {
            return new Promise(async resolve => {
                ...
                const action = this.actions[this.index],
                        prms = [Themify.loadJs('https://cdnjs.cloudflare.com/ajax/libs/lottie-web/5.12.2/lottie.min.js', !!window.lottie), ...this.loadChain()];
        ...
        async init() {
            if (this.isLive()) {
                ...
                if (!action.segment) {
                    ...
                    if (state !== 'click' && state !== 'hover' && action.sel && action.sel !== 'parent') {
                        action.sel = '';
                    }
        ...
        getParent() {
            return this.el.closest('a,button') || this.el.closest('em,i') || this.el;
        }
        async autoplay() {
        ...
        click(type, max) {
            return new Promise(resolve => {
                            ...
                            if (!sel || !e || e.target.closest(sel)) {
                                ...
                                if (!isWorking) {
                                    ...
                                    if (count >= max) {
                                        ...
                                        if (link && !Themify.is_builder_active) {
                                            const href = link.getAttribute('href');
                                            if (href && href !== '#') {
                                                Themify.triggerEvent(target, ev);
                                            }
                                        }
                                        count = el = sel = ev = passive = null;
                                        resolve();
                                    }
                                    isWorking = null;
                                }
                ...
                if (type !== und && this.player.isPaused) {
                    const items = sel ? doc.querySelectorAll(sel) : [el];
        ...
        hover() {
            const sel = this.action.sel;
            return this.click((sel && sel !== 'parent' ? 'pointerover' : 'pointerenter'));
        }
        hold(type) {
            return new Promise(resolve => {
                let segment = this.action.segment.slice(),
                        ...
                        el = this.action.sel === 'parent' ? this.getParent() : this.el;
                ...
                const leave = async () => {
                    ...
                    try {
                        this.player.trigger('reject');
                        el.tfOff('pointerleave', leave, {passive: true, once: true})
                ...
                el.tfOn('pointerenter', enter, {passive: true});
                if (this.player.isPaused && el.matches(':hover')) {
                    enter();
                }
            });
        }
        ...
        seek() {
            return new Promise(resolve => {
                const el = this.action.sel === 'parent' ? this.getParent() : this.el,
                        enter = () => {
                    let box = el.getBoundingClientRect(),
                            ...
                            prevFrame = 0;
                    const leave = e => {
                        el.tfOff('pointermove', move, {passive: true})
                                .tfOff('pointerleave', leave, {passive: true, once: true});
                        if (req) {
                        ...
                        requestAnimationFrame(() => {
                            w = frameLength = prevFrame = req = max = min = isReverse = null;
                        });
                    },
                            move = e => {
                    ...
                    el.tfOn('pointerleave', leave, {passive: true, once: true})
